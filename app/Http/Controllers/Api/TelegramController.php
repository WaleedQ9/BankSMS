...
class TelegramController extends Controller
{
    ...
    private function handleReport(): void
    {
        ...
        $cycleLabel = $cycle->is_open
            ? 'بدأت ' . $cycle->start_date->format('d/m') . ' — الراتب المتوقع ' . $this->cycleService->getExpectedSalaryDate($cycle)->format('d/m')
            : $cycle->start_date->format('d/m') . ' - ' . $cycle->end_date->format('d/m');
        $text .= "الدورة: {$cycleLabel}\n";
        $text .= "الفترة: {$week->week_number} من 4\n\n";
        ...
        foreach ($budgets as $budget) {
            ...
            if ($cat->show_in_weekly) {
                ...
                if ($weeklyAllowance !== null && $weeklyAllowance > 0) {
                    ...
                    $text .= "  الفترة الحالية: " . number_format($weeklySpent, 0) . " / " . number_format($weeklyAllowance, 0) . " ريال ({$weeklyPct}%)\n";
                }
            }
...
1. افحص حالة الدورة وفترة الصرف الحالية، ثم راجع كل بند غير ادخاري.
2. صنّف التنبيهات بهذا الترتيب:
   أ) تجاوز فعلي: remaining أقل من صفر.
   ب) تجاوز في فترة الصرف: weekly.remaining أقل من صفر.
   ج) اقتراب من الحد: percent_used بين 85 و99، أو weekly.percent بين 85 و99 مع بقاء أيام في الفترة.
3. رتّب الأولويات: التجاوز الفعلي، ثم تجاوز الفترة، ثم الاقتراب من الحد. داخل المستوى نفسه قدّم الأكبر مبلغاً.
...
- weekly=null تعني أنه لا توجد حصة لفترة الصرف للبند؛ لا تستنتج منها أي شيء.
...
📊 تقريرك المالي — الفترة [رقم الأسبوع] من 4
...
أسلوب العرض النهائي — يحل محل أي تنسيق سابق في هذه التعليمات:
📊 تقريرك المالي للفترة [رقم الأسبوع] من 4

1️⃣ تقييم عام للوضع المالي:
فقرة إنسانية قصيرة من جملتين. اذكر ما يحتاج الحذر بلهجة متوازنة، ووضّح أن المتبقي داخل البنود مقيد وليس رصيداً حراً عند الحاجة فقط.

2️⃣ بنود تحتاج انتباه عاجل (متجاوزة أو قريبة):
من 2 إلى 5 أسطر. استخدم 🔴 للتجاوز الفعلي و⚠️ للاقتراب من الحد أو تجاوز فترة الصرف. اجمع البنود المتشابهة عند الحاجة، ولا تذكر بنداً وصل 100% فقط بلا تجاوز.

3️⃣ نصائح عملية للأيام المتبقية:
3 أو 4 نصائح مرقمة، واقعية ومتصلة بالأولويات. لا تقترح استخدام فائض بند لغرض بند مختلف، ولا تبنِ نصيحة على افتراض أن رصيد بند ما متاح لغير غرضه.

4️⃣ اقتراح لتحسين الميزانية القادمة:
نقطة أو نقطتان بعلامة -، مبنيتان على التجاوزات أو النمط الفعلي فقط.

استخدم صياغة طبيعية ودافئة مثل «يحتاج إلى حذر» و«خفّض الإنفاق»، بدلاً من الأسلوب التقني أو الجاف. لا تكتب أي تعليمات أو عبارات تفسيرية قبل عنوان التقرير.
PROMPT;
        ...
        $userPrompt = <<<PROMPT
حلل بيانات الدورة التالية وفق التعليمات. أعد التقرير النهائي فقط، ولا تنسخ أو تكرر أي جملة من هذا الطلب.
